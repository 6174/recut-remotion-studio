...
[POS]: Remotion Studio 的唯一运行时创作 Skill；镜头表达以 Shot Language v3 为语义入口。
...
开始设计前按顺序读**三件参考**；只要成片使用 Three、镜头层特效、页面/设备入镜或放大镜，再额外读第 4 项：
...
4. **Shot Language**：`{paths.appKitPath}/skills/remotion-studio/references/shot-recipes/camera/README.md`（camera、surface、attention 的组合合同）；根据选中的 preset 继续读取对应配方。
...
- **默认根**：`ProjectVideo` 包 `ThreeVideoCanvas`；内容镜头用 `ShotGraph` 的声明式 plan（`content`/`camera`/`surface`/`effect`/`transition`/`lens`/`ambient`）。
- **内容层渲染在真实 React 树内**：`renderContent(shot)` 返回 world 内容，`ShotGraph` 用 `<Sequence from={shot.start}>` 提供镜头局部帧，因此内容内部可以正常用 Remotion hook（`useCurrentFrame`/`spring`/`interpolate`）。标题、说明、HUD 可通过 `Scene.screenKind` + `SceneScreenLayer` 在 Three 输出后作为 screen layer 叠加；字幕始终位于两层之上。
- **效果写材质，不写 DOM**：surface 的 `bend` / `cornerCurl` / `cloth` 发生在实际 Three 网格，`effect: "displacement"` 可与它们组合；放大镜/玻璃/glitch/CRT/胶片/气泡/模糊走 attention/post；云/雾/粒子氛围走 ambient。参数只能取 `catalog.json` effects 里 `material.schema` 声明的语义参数。
...
## 镜头表达：Shot Language v3

只要任务涉及推近、拉远、平移、三维视角、焦点、景深、放大镜、翻页、browser/device shell 或 flip，先读 `references/shot-recipes/camera/README.md`，再按需读具体配方。先用一句镜头语义确定观看意图，再选择当前已经存在的实现；不从 CSS 参数或 shader 名称反推镜头。

```text
意图 -> 空间基底 -> shell -> pose/geometry -> camera -> 主体 -> attention -> 节奏 -> 排他项
```

- 每镜头先在 `catalog.json.effects` 选择一个 `engine="three-camera"` preset；它已经给出可用的 `camera` + `surface` 描述和时长约束。先替换 `subject.anchor` 为实际目标，再按内容小幅调参，不能只复制 CSS 动画。
- `surface` 负责被拍物：`shell`（plain/browser）、`position`、`rotation`、`scale`、`bend`、`cornerCurl`、`cloth`；`camera` 负责观看者。两者可组合，但各自只允许一个主动作，默认约 24--32f 完成落位，之后必须 hold 阅读。
- 几何与像素的组合顺序固定为 `shell → pose → bend/cornerCurl/cloth → displacement → focus/lens → grade`。`displacement` 可和 surface 几何叠加；`magnify`、`glass`、`bubble` 是同层强光学主角，彼此互斥。
...
- `single-plane` 可以做真实 Three camera 的位置/FOV 与 surface 倾斜、全页/单角弯曲，但不能声称获得页面内部的真实景深；没有显式 `depth-layers` 时，focus blur 是屏幕空间注意力处理。
...
- `ShotGraph` 已支持 `CameraMoveDescriptor` + `SurfaceMoveDescriptor`：前者以 `subject.anchor` + 局部进度 keyframes 驱动真实 Three camera，后者以同一进度驱动内容 mesh / shell 的姿态与曲面。优先复用 catalog 的 10 个已验证组合；多平面真实景深与有序 post chain 仍按 `dev/2026-08-11-three-camera-shot-system-design.md` 后续演进，不得假称已完成。
...
2. **World 上下文**：会话开始时读一次 `workflow.context`，同一会话复用。若它返回 `creationContext`（非 null），该世界 Canon 是权威：遵守 `constraints` 的 always/never，优先使用 `references` 里登记的参考 Asset（以真实 `assetId` 经 `resolveMediaUrl` 引用并登记到 `composition.assets`），`entities.characters/styles` 中的不可变设定不得擅自改写；Canon 之外的创作新设定标记为“提议”，经用户确认后才写回 World，绝不自动把生成结果变为 Canon。未绑定的项目返回 `creationContext: null`，完全按现有无 World 流程工作。
3. **编辑代码**：本会话尚未读过 `workflow.context` 时先调用它看阶段、`workspace` 状态与绝对路径 `paths.workspacePath`（已读过则直接复用），再 `workspace.ensure` 确保工程就绪。然后读上述四件参考，用**原生文件工具**（Read/Write/Edit/Glob）直接读写 `${paths.workspacePath}` 下的 `src/compositions/ProjectVideo.tsx` 与 `src/Root.tsx`。**设计就是写代码**：不存在 `code.read`/`code.write`/`composition.save` 之类的结构契约，也不要执行 `render.export`。
...
3. **`references/captions.md`** —— 字幕主题的 Remotion 介质映射（薄适配层，决策规则见 `service/skills/recut-director/references/captions`）。用户选择主题后，把主题 id 设为 `palette.captionTheme`，或直接用 `<CaptionTheme theme="…">`；旁白用 `buildCaptionsData` 生成逐词字幕；字幕层级/安全区/强调词的决策以全局为准。
4. **创作参考** —— `references/creation-modes.md` 只在需要自主创作、共同创作或单镜头时决定工作方式；随后按任务读取对应资料：`production-workflow.md`（完整制作流程）、`shot-recipes/camera/README.md`（只要使用镜头表达必须先读）与选中配方（`shot-recipes/` 为 `recut-director（references/shot）` 的薄适配层，意图见全局）、`sequence-patterns/`（桥段骨架）、`aesthetic-rules.md`（薄适配层，决策规则见 `service/skills/recut-director/references/motion`，仅保留 Remotion 高清栅格化/有效字高等约束）、`sound-design.md`（薄适配层，决策规则见 `service/skills/recut-director/references/sound`，仅保留 SFX 声明式表/相对钉帧/`assets/audio/` 清单）与 `music-beat-sync.md`（薄适配层，决策规则见 `service/skills/recut-director/references/editing`，仅保留 `beatF`/`SHOTS` 写法）、`final-review.md`（独立终检）。`paper-ink-product-promo.md` 仅在用户明确要纸墨编辑风产品片时读取。`directing.md` 为全局 `recut-director 的 references/motion|editing|shot` 的薄适配层，仅保留 `interpolate`/`spring`/`ShotDescriptor` 介质映射。
...
- 代码引用的 assetId 集合变化时调用 `composition.assets({assetIds})` 登记全部引用，导出才能物化；集合未变时不重复调用。漏登记的画面在导出里会是空。
...
画面有主次、有证据、有停顿；字幕清晰可读、与旁白逐词对齐；一个 content 场景只说一个新信息。动效取舍的决策规则见全局 `service/skills/recut-director/references/motion`（嗓音/呼吸/高光）与 `recut-director（references/editing）`（节拍/切点）；Remotion 落法见 `references/directing.md` 薄适配层（`interpolate`/`spring`/`ShotDescriptor`/`hold`）：一种动画手法全片只当一次主角、关键信息落定后必须呼吸（hold 至少 0.5s）、开场三秒内给钩子、不堆砌装饰性光效。
