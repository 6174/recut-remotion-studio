...
directing.md: 全片镜头语言、节奏和确定性渲染速查。
production-workflow.md: 从产品理解、分镜到交付的完整制作流程。
collaborative-creation.md: 共同创作中需要用户确认的最少决策点。
shot-recipes/: 按镜头类型整理的运动配方；只提供动作语言，实际实现以 remotion-kit 为准。
...
gpu-composition.md: Three-first GPU 合成的架构契约。

依赖边界
`SKILL.md` 只按任务读取本目录资料；成片结构、场景原语、组件、材质和实际导出路径都以 `@recut/remotion-kit` 与其 `catalog.json` 为单一真相源。
