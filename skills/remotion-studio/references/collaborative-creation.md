...
1. 先给出最多 3 个根据当前项目生成的文字方向。每个方向说明：名称、一句话
   ...
   只有产品确实适合纸张、编辑部或叙事质感时，才把 纸墨编辑风产品片 列为候选；不能
   因为仓库里有它就固定加入。
...
结合动效 tokens、镜头配方 参考样片判断。若动效差异无法靠文字和样片判断，再做
短的运动测试。用户已经提供严格品牌规范、明确参考片、直接选定 纸墨编辑风产品片，或
明确授权 Agent 自选并跳过视觉确认时，可以记录理由后跳过 styleframe 对比。

## 4. 功能到镜头映射

先列产品功能，再扫描 `shot-recipes/` 各卡 frontmatter 的用途、能量、建议
时长和限制。给每个必须展示的功能推荐首选和备选镜头：

| 功能 | 首选镜头 | 备选镜头 | 画面要表达什么 | 需要的页面状态 |
|---|---|---|---|---|

用户可以接受、替换、删除或授权 Agent 自选。确定映射后，完整读取选中卡片，并从
`@recut/remotion-kit/components` 或场景 `primitives.tsx` 选择可复用实现；没有合适能力时
才新增代码，并把新增范围和验证风险写进设计 spec。
...
用户可以提供一个镜头名称，或 `镜头名 · 变体`。名称只用于定位本地
`shot-recipes/` 中的配方，不是另一个实现入口。

收到镜头名后按以下顺序处理：

1. 在 `shot-recipes/` 中定位并完整读取匹配卡片；名称不匹配时，报告最接近的本地卡片，不能猜测一个不存在的效果；
2. 读取卡片的动作、时值、遮罩和“已知坑/命门”，再对照场景 `primitives.tsx` 与 `{paths.appKitPath}/catalog.json` 选择实际 kit 组件或材质；
3. 在设计 spec 记录卡名、选用的 kit 实现和为目标产品调整的参数；
4. 只替换真实素材、布局坐标、文案和品牌 token，保留卡片定义的动作语法与关键时值；
5. 卡片没有对应 kit 能力时，明确新增实现的范围与验证计划，不声称是在复刻已有组件。
...
- 只有卡名：使用该卡默认配方；如果卡片列出多个变体，按前后镜头与产品气质推荐一个。
- `卡名 · 样式名`：优先使用用户点名的变体，不擅自换式。
- 名称不存在：报告无法匹配，并给出最接近的本地卡片；不要凭名称臆造。

严禁只根据卡名含义重新写一套近似动画。先读配方，再用 remotion-kit 的实际实现
落地。用户选中多个镜头后，把它们与产品功能映射，再继续生成完整分镜。
...
把批准分镜转成帧级时间轴，再进入 `production-workflow.md` 阶段 4 的最终素材采集、阶段 5
逐镜头实现、阶段 6 声音设计和阶段 7 终检。

将用户确认后的产品简报、需求到执行决策表、视觉方向/tokens、styleframe 或跳过理由、
功能到镜头映射、镜头配方 卡名与具体变体、最终分镜和数据合规口径保存为稳定的设计 spec
