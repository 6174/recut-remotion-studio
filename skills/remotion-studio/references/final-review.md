...
- 功能到镜头映射；
- 用户指定或 Agent 在设计 spec 中记录的镜头卡名与具体变体；
- 选中的 `shot-recipes/` 卡片；
- 对应场景的 `primitives.tsx`、kit 组件或材质源码；
- 可用的关键帧或参考画面；
- 最终分镜、字幕、页面状态和 SFX 计划；
- `aesthetic-rules.md`；
- 数据合规口径。

缺少某项输入时标记“无法验证”，不要自行猜测制作基准。配方卡、场景原语和 kit
源码互相冲突时，以实际 kit 源码为准，并在报告中明确差异。
...
- V3：是否偏离确认的制作基准，漂移到与选题无关的默认风格；
- V4：禁用的颜色、效果或品牌特征是否被使用。

### S — 镜头卡与实现

- S1：功能映射中的镜头卡是否实际出现在对应镜头；
- S2：用户点名 `卡名 · 样式名` 时，配方、所选场景原语或 kit 组件是否指向同一变体，成片是否使用该变体；
- S3：对照配方与实际实现，是否保留动作语法、关键时值、缓动关系和遮罩时机；
- S4：是否违反卡片标注的“已知坑/命门”；
- S5：适配后的产品截图、坐标和品牌 token 是否自然。
- S6：新增实现是否得到用户明确同意或记录了推导依据，并在报告中标注验证风险。

不要求与参考画面像素级一致。检查的是运动语法和关键参数是否忠实，同时
