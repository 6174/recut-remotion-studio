...
八阶段，方向性问题不进昂贵的逐镜头阶段。
实证细节来自多支已交付宣传片的复盘；纸墨编辑风的可选参考见 `paper-ink-product-promo.md`。
...
直接使用纸墨编辑风产品片不进入本流水线，按 `paper-ink-product-promo.md` 的替换
...
1. 根据当前项目生成最多 3 个文字方向。每个方向附一组**动效性格 tokens**
   （推导法见下方“品牌→动效参数”表），但不要把 纸墨编辑风产品片 作为固定选项。
...
4. 用 Playwright/Puppeteer 截图验证字体、色板、材质、构图、光感与信息密度；
   缓动和运镜速度仍以 tokens、镜头配方 样片或短运动测试判断。若已有严格品牌规范
...
2. 复制 `scripts/capture-product.mjs` 进项目，改 BASE_URL 与选择器，
   跑一次产出三件套：
...
注册 Composition；项目 workspace 已冻结 `remotion-kit/`，直接从
`@recut/remotion-kit/components` 导入动态组件；走模板路线则以所选场景模板起步）。
...
1. **每镜头先解析再实现**（硬规则，不可跳）：读对应 `shot-recipes/` 卡片全文 →
   读所选场景的 `primitives.tsx` 或 kit 组件源码 → 在 `catalog.json` 确认组件/材质能力。
...
2. **独立终检（必做）**：派一个干净上下文的 subagent 做第三方审查。
   ...
   功能到镜头映射、选中的镜头配方卡、对应的 kit 组件或场景原语、最终分镜、
   `final-review.md` 和 `aesthetic-rules.md`。
3. 审查 Agent 按 `final-review.md` 检查产品目标、功能完整性、视觉方向一致性、
   ...
   镜头卡适配不要求像参考画面那样逐像素相同，但必须保留动作语法、关键时值、
   已知坑/命门参数和用户点名的具体变体。
