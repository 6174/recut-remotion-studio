...
成员清单
catalog.json: 成片模板、字幕、画布、组件与镜头层效果（materials + Camera Language presets）的唯一目录；每个模板描述可交给 AI 的完整动效节奏。
manifest.json: kit 版本元数据。
package.json: kit 的导出边界与 peer dependency。
src/: 可复用的字幕、效果、调色板、模板、动态组件、html-canvas 交互层、GPU 材质与 three 运行时桥。
...
效果由材质实现，镜头由 ShotGraph 声明式模型与 CameraDirector 装配。HTML-in-Canvas 交互（src/html-canvas/）只提供帧驱动互动脚本与
