 ...
 * [POS]: scenarios/product-launch 的模板代码（Three-first 模式）。世界层（beats/primitives/UI）
 *        经 HtmlSurface 光栅化为 GPU 纹理；screen beat 与字幕在 Three 输出后叠加；ui-detail 的设置页先以倾斜姿态快速落位，再由 CameraDirector / ShotGraph lens（magnify 材质）检查；
 *        交互（hover/pressed/cursor）由帧驱动互动脚本注入内容表面，不绘制 spotlight 遮罩。
...
import { buildGpuScenePlan, createSceneContent, SceneCaptionOverlay, SceneScreenLayer, ScreenTitleBeat } from "../../_shared/GpuSceneEngine";
...
export const buildProductLaunchScenes = (props?: { topic?: string; productName?: string }): Scene[] => {
  ...
  return [
    ...
    { id: "ui-detail", kind: "ui-detail", title: "导出画质，一键开启", subtitle: "镜头进入设置面板，标题与说明将保持屏幕空间。", kicker: "06 · UI", ctaLabel: "导出", screenKind: "screen-title", durationSec: 6 },
...
/** 按场景时间轴计算 ui-detail beat 的帧驱动互动脚本（click → magnify 的鼠标轨迹）；
...
/** 产品 UI 的唯一空间镜头：推近到操作目标，随后由 magnify 检查同一个导出按钮。 */
const PRODUCT_UI_INSPECT_CAMERA: CameraMoveDescriptor = {
  verb: "push-in",
  subject: { anchor: EXPORT_BUTTON_UV },
  keyframes: [
    { at: 0, position: [0, 0, 8], fov: 34 },
    { at: 0.16, position: [0.18, -0.1, 5.8], fov: 30, easing: "ease-out" },
    { at: 1, position: [0.18, -0.1, 5.8], fov: 30, easing: "linear" },
  ],
};

/** 单张设置页从左后方带轻微 Dutch 倾斜落到镜头前；6 秒镜头前约 1.1 秒完成，余下时间用于阅读。 */
const PRODUCT_UI_LAND_SURFACE: SurfaceMoveDescriptor = {
  shell: "browser",
  keyframes: [
    { at: 0, position: [-0.72, 0.32, -1.35], rotation: [0.09, -0.22, -0.05], scale: [0.78, 0.78, 1], bend: 0.38 },
  ...
  buildGpuScenePlan({ scenes }, fps, {
    ...
    lensFor: (scene) =>
      scene.kind === "ui-detail"
        ...
        : undefined,
    cameraFor: (scene) =>
      scene.kind === "ui-detail" ? PRODUCT_UI_INSPECT_CAMERA : undefined,
    surfaceFor: (scene) =>
      scene.kind === "ui-detail" ? PRODUCT_UI_LAND_SURFACE : undefined,
    optionsFor: (scene) =>
  ...
  return (
    <AbsoluteFill>
      ...
      <SceneScreenLayer
        palette={PRODUCT_LAUNCH_PALETTE}
        scenes={resolvedScenes}
        beats={{ "screen-title": ScreenTitleBeat }}
      />
