...
- `vite.config.ts` root=workspace、publicDir=preview/（props.json）；`@tailwindcss/vite` 处理 Tailwind
- `vite-server.js` 启动 Vite dev server，端口写 `serve/status.json`（预览 = HMR）
...
- `src/compositions/ProjectVideo.tsx` 成片模板：SCENES（内容）+ 渲染层（编辑对象）
