 ...
 * [OUTPUT]: 对外提供独立 Vite 入口，展示完整 Remotion 产品短片、capture adapter 切换和性能诊断
...
import {
  CompositionGraphComposition,
  GRAPH_DURATION_IN_FRAMES,
} from "./composition";
  ...
  <ol className="graph-tree">
    {compositionGraph.map((node) => (
      <li
        ...
        key={node.id}
        style={{ marginLeft: node.parentId ? 20 : 0 }}
      >
...
const CaptureColumn: React.FC<{
  ...
  const mean =
    measurement.samples.length === 0
      ? null
      : measurement.samples.reduce((total, value) => total + value, 0) /
        measurement.samples.length;
  const p95 = percentile(measurement.samples, 0.95);
  const state = measurement.verified
    ? "VERIFIED"
  ...
  return (
    <article className="benchmark-column">
      <div className="benchmark-column-heading">
        <span>{label}</span>
        <output
          className={`capture-state capture-state-${measurement.status}`}
      ...
      <div className="benchmark-samples">
        {measurement.samples.length} / {SAMPLE_WINDOW} samples
      </div>
      {measurement.verified ? (
        <div className="benchmark-proof">
          paint {measurement.paintCount} · drawElementImage(DIV)
        </div>
      ) : null}
    </article>
  );
...
const BenchmarkPanel = () => {
  ...
  useEffect(() => {
    const fpsListener = (event: Event) =>
      setFps((event as CustomEvent<{ fps: number }>).detail.fps);
    const captureListener = (event: Event) => {
      ...
      const samples =
        typeof detail.duration === "number"
          ? [...previous.samples, detail.duration].slice(-SAMPLE_WINDOW)
          : previous.samples;
      const status =
        typeof detail.duration === "number"
          ? "recording"
      ...
      measurementsRef.current = {
        ...
        [detail.adapter]: {
          ...
          verified: detail.verified ?? previous.verified,
          paintCount: detail.paintCount ?? previous.paintCount,
        },
      };
    };
    ...
    return () => {
      ...
      window.removeEventListener(
        "composition-graph-html-metrics",
        captureListener,
      );
    };
  }, []);
  ...
  return (
    <section aria-label="HTML capture benchmark" className="benchmark-panel">
      <div className="benchmark-heading">
        <div>
          <p>CAPTURE BENCHMARK</p>
          <span>rolling CPU copy time</span>
        </div>
        <output className="benchmark-fps">
      ...
      <div className="benchmark-grid">
        <CaptureColumn
          ...
          measurement={measurements["foreign-object"]}
        />
        <CaptureColumn
          label="HTML-in-Canvas"
          measurement={measurements["html-in-canvas"]}
        />
      </div>
    </section>
  );
...
const Lab = () => {
  ...
  const [htmlRasterizer, setHtmlRasterizer] = useState<Adapter>(() =>
    supportsHtmlInCanvas() ? "html-in-canvas" : "foreign-object",
  );
  ...
  return (
    <main className="lab-shell">
      <header className="lab-header">
        <div>
          <p>RECUT / COMPOSITION LAB</p>
          <h1>Remotion + Three + HIC</h1>
        </div>
        <div className="runtime-status">
          <span>120 SECONDS</span>
          <span>ALL VISIBLE NODES IN THREE</span>
          <span>HIC + CANVAS UI</span>
          <label className="lab-select">
            <span>capture</span>
            <select
              aria-label="HTML capture adapter"
              onChange={(event) =>
                setHtmlRasterizer(event.target.value as Adapter)
              }
              value={htmlRasterizer}
            >
              <option value="foreign-object">foreignObject</option>
              <option disabled={!nativeAvailable} value="html-in-canvas">
                HTML-in-Canvas
              </option>
            </select>
          </label>
          <label className="lab-toggle">
            <input
              checked={htmlAnimation}
              onChange={(event) => setHtmlAnimation(event.target.checked)}
              type="checkbox"
            />
            animate HTML
          </label>
      ...
      <section className="film-main">
        <div className="player-frame">
          <Player
            ...
            component={CompositionGraphComposition}
            compositionHeight={1080}
            compositionWidth={1920}
            controls
            durationInFrames={GRAPH_DURATION_IN_FRAMES}
            fps={30}
            inputProps={{ htmlAnimation, htmlRasterizer, magnify }}
            loop
