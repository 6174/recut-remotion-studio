/**
 * [INPUT]: 依赖 Three Texture、Remotion transition progress/time、StorePeel 的 PeelSticker shader 与 R3F ShaderMaterial
 * [OUTPUT]: 对外提供 HtmlStorePeelMaterial，以上游相同 cylinder curl、adhesive back 与双层 shine 揭示当前 HTML texture
 * [POS]: composition-graph 的 StorePeel Transition Node；从 opaque scene texture 取样，不创建额外 DOM 或独立时钟
...
// Ported from Bonus/SaleStickerComposition.tsx (PeelSticker FS), with the
// full composition texture taking the place of the die-cut sticker canvas.
...
export const HtmlStorePeelMaterial: React.FC<HtmlStorePeelMaterialProps> = ({
  ...
  useLayoutEffect(() => {
    ...
    material.current.uniforms.uProgress.value = 1 - reveal;
    material.current.uniforms.uBandT.value =
      -shineRange + 2 * shineRange * reveal;
    material.current.uniforms.uResolution.value.set(width, height);
  }, [height, progress, shineRange, time, width]);
