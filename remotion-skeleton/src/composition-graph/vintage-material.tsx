/**
 * [INPUT]: 依赖 Three Texture、Remotion frame/fps 派生时间、Vintage/gl.ts 的 film shader 与 R3F ShaderMaterial
 * [OUTPUT]: 对外提供 HtmlVintageMaterial，以 gate weave、grain、scratch、dust、leak、vignette 与 flicker 渲染 HTML texture
 * [POS]: composition-graph 的 Vintage Effect Node；保持上游离散 24fps grain，时间只由 Remotion 注入
...
// Ported from remotion-dev/html-in-canvas/src/Vintage/gl.ts (FS).
...
export const HtmlVintageMaterial: React.FC<HtmlVintageMaterialProps> = ({
  ...
  const uniforms = useMemo(
    () => ({
      ...
      uGrain: new THREE.Uniform(0.126),
      uVignette: new THREE.Uniform(0.6),
      uWarmth: new THREE.Uniform(0.28),
      uFade: new THREE.Uniform(0.385),
    }),
