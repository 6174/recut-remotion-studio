/**
 * [INPUT]: 依赖 Three Texture、Remotion frame/fps 派生时间、Crt/gl.ts 的光学模型与 R3F ShaderMaterial
 * [OUTPUT]: 对外提供 HtmlCrtMaterial，以 barrel、scanline、RGB aperture、vignette 与 flicker 渲染 HTML texture
 * [POS]: composition-graph 的 Crt Effect Node；shader 仅创建一次，Remotion 帧只更新 uniform
...
// Ported from remotion-dev/html-in-canvas/src/Crt/gl.ts (FS).
...
export const HtmlCrtMaterial: React.FC<HtmlCrtMaterialProps> = ({
  ...
  const uniforms = useMemo(
    () => ({
      ...
      uScanIntensity: new THREE.Uniform(0.24),
      uVignette: new THREE.Uniform(0.68),
    }),
