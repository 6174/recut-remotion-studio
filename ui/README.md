...
src/studio.tsx: 工作室工作面：左预览 + 右侧创作列；以「选择成片模板」为主入口，微调只保留表达特效、内容组件、素材、字幕与画布，统一走「参数选择 → 可编辑 Prompt → Agent」，并以 FineTune 的 ready 合同禁用不满足资源或原生能力的提交。
src/fine-tunes/: 当前成片的微调动作与 Prompt 生成模块；所有 UI 组件、类型和目录名均用 FineTune 区别于真正的成片模板。工作台动作顺序固定为「表达特效 → 内容组件 → 使用素材 → 其他微调」；EffectsFineTune 在同一弹框按「Three 镜头 / 后处理 / 转场 / 环境」分组，并以紧凑网格小卡片展示：用户只选择效果，参数 schema 自动附入 Prompt，Agent 决定 descriptor.camera 或 effect/transition/ambient 的语义挂载与最终参数；CaptionsFineTune 用白色网格 Player 检验无底框字幕动效。
