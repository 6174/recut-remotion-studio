...
src/studio.tsx: 工作室工作面：左预览 + 右侧创作列；以「选择成片模板」（模板 = 场景）为主入口，配当前成片编辑工具，统一走「参数选择 → 可编辑 Prompt → Agent」。
src/scenarios/: 成片模板（TemplateScenario）与当前成片编辑的资源选择、Prompt 生成模块；CaptionsScenario 以分组卡片缩小范围，并用可拖动时间轴的真实大预览检验无底框字幕动效。
...
设计规范
App 使用 Recut 设计系统语义 token（浅色内容面、主命令按钮与紧凑工具控件）；组件只组合 Tailwind 工具类与本地 shadcn 原子（components/ui/），不另建 CSS 主题。宽画布留给 Remotion Studio 预览 iframe，右侧列保持 24rem 信息密度；创作以「选择成片模板」开始——模板 = 场景（catalog.scenarios），Agent 读取对应场景技能与模板代码后重写这支视频的结构、镜头与组件组合；随后才提供设计风格、字幕、画布、SRT、shotcraft 组件和素材重剪等局部编辑工具。更换资源会生成新的默认 Prompt。字幕一律无底框：可用字重、描边、阴影、柔焦和动效增强可读性，但不得用色块、玻璃或卡片包裹文字。字幕选择先用分组卡片缩小范围，再通过可播放、可拖动的真实大预览确认动效；小卡永远不承担动效检验。素材不作为孤立的技术信息面板，而是通过成片模板或「用素材重剪」选择并传递 assetId。导出、构建、重启与重置均为次级工具，其中导出参数和渲染状态仅在模态框展开；终端/日志分栏默认显示终端，日志切换保留各自状态。
