...
成员清单
compositions.tsx: 预览合成分发；将字幕、成片模板（composition → 场景真实组件）和 shotcraft 组件严格区分。
PreviewCard.tsx: 单项 Remotion Player 预览，静音自动循环播放；全铺字幕预览在加载和主题切换时维持白底，缩略图用独立字号渲染，避免 1920 画布缩小后文字不可读。
